...
from src.domain.exceptions.base import BadRequest
...
class UserPassword:
    ...
    def _validate(self) -> None:
        if len(self.value) < self.MIN_LENGTH:
            raise BadRequest(f"Пароль должен быть длиной минимум {self.MIN_LENGTH} символов.")

        if not re.search(r"[a-z]", self.value):
            raise BadRequest("Пароль должен содержать хотя бы одну строчную букву.")

        if not re.search(r"\d", self.value):
            raise BadRequest("Пароль должен содержать хотя бы одну цифру.")
    ...
    def verify(self, plain_password: str) -> bool:
        if not plain_password:
            raise BadRequest("Пароль для проверки не может быть пустым.")
